class MemberTestsController < ApplicationController
  ...
  def index
    ...
    @member_tests = current_company.member_tests.includes(:member, test: :test_group).where(status: @status_filter).joins(:member).merge(Member.order(:name)).merge(TestGroup.order(:name)).merge(Test.order(:module))
  end
